...
import {CategoryScale, LinearScale, BarElement, BarController, Chart as ChartJS, Title, Tooltip, Legend} from "chart.js";

// Register all required component for preventing errors
...
function Infografis() {
  ...
  const ageGroupStats = ageGroups.map((group) => {
    ...
    const males = residents.filter((r) => r.jenisKelamin === "L" && r.umur >= start && r.umur <= (end || r.umur)).length;
    const females = residents.filter((r) => r.jenisKelamin === "P" && r.umur >= start && r.umur <= (end || r.umur)).length;
      ...
      <div className='py-5 content'>
        <div className='container text-center'>
          ...
          <p className='lead py-2'>
            Memberikan informasi lengkap mengenai karakteristik demografi penduduk suatu wilayah. Mulai dari jumlah penduduk, jenis kelamin, agama,
          ...
          {Object.keys(occupationStats).length === 0 ? (
            ...
            <ul className='list-group mb-5'>
              {Object.entries(occupationStats).map(([job, count]) => (
                <li className='list-group-item d-flex justify-content-between align-items-center' key={job}>
                  {job}
...
function StatCard({title, value, color}) {
  return (
    <div className='col-6 col-sm-6 col-md-3'>
      <div className='card text-center shadow stat-card h-100' style={{borderBottom: `4px solid ${color}`}}>
        <div className='card-body'>
